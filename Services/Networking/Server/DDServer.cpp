...
DDDatabaseQuery DDServer::ParseRequest(const char* requestData)
{
    DDDatabaseQuery query;

    // TO DO: add function to DD string for locating characters
    // Will use new DDString methods to split up the query data and parse it

    return query;
}
void DDServer::SendResponse(SOCKET client)
{
    const char* body = "<html><body>Hello from DB server!</body></html>";
...
DDDatabaseQuery DDServer::WaitForRequest(DDString portToListenOn)
{
        ...
        while (true)
        {
            ...
            if (client != INVALID_SOCKET)
            {
                ...
                if (bytesReceived > 0)
                {
                    ...
                    DDDatabaseQuery request = ParseRequest(buffer);
                    SendResponse(client);
